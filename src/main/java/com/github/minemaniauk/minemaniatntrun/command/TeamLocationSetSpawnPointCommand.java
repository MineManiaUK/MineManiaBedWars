...
public class TeamLocationSetSpawnPointCommand implements CondensedCommand {
    ...
    public @Nullable CommandStatus onPlayerUser(@NotNull PlayerUser user, @NotNull CommandArguments arguments, @NotNull CommandStatus status) {
        ...
        teamLocation.setSpawnPoint(location);
        arena.save();

        // Send a confirmation message.
        user.sendMessage("&a&l> &aThe spawn point of the team location &f" + teamLocation.getColor().getName() + " &ais now set to &f" + location);
        return new CommandStatus();
    }
